...
class _ScheduleDoctorPageState extends State<ScheduleDoctorPage> {
        ...
        body: BlocListener<ScheduleBloc, ScheduleState>(
          listener: (context, state) {
            if (state is ScheduleLoading) {
              ...
              buildLoadAllSchedule(state.dateTimeCreated);
              ...
              if (listData.length > 0) {
                listDataDefault.forEach((item) {
                  final schedule = data.firstWhere((element) => element.timeBook == item.timeBook, orElse: () => null);
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Row(
                ...
                children: <Widget>[
                  SizedBox(width: 25,),
                  Text('List schedule: ${listData.length}', style: TextStyle(
                    fontSize: 16,
  ...
  _buildItemSchedule(List<ScheduleModel> list) {
    list.sort((a, b) => a.timeBook.compareTo(b.timeBook));
    if (list.length == 0) {
      return Center(
       child: Text('Empty'),
      );
          ...
          children: <Widget>[
            Row(
              children: <Widget>[
                ...
                Spacer(),
                Text(item.timeBook.getTypeTimeSchedule(), style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: textColor),),
              ],
            ),
            SizedBox(height: 5,),
                ...
                child: ButtonCustom(
                  ...
                  onPressed: () async {
                    if (widget.timeSelected != null && widget.timeSelected.id != null) {
                      ...
                      toast('Please choose time', gravity: ToastGravity.BOTTOM);
                    }
                  },
                ),
              ),
                        ...
                        child: FlatButton(
                          ...
                          onPressed: () {
                            if (_currentTimeValue.length != 0) {
                              final item = widget.listScheduleBooked.firstWhere((element) => element.dateTime == widget.dateTimeSelected.millisecondsSinceEpoch
